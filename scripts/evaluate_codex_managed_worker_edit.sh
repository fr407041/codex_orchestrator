...
RUN_ROOT="${REPO_ROOT}/orchestrator-codex/manual-managed-edit"
...
cat > "${JOBS_DIR}/job-001.json" <<'EOF'
{
  ...
  "title": "Rewrite placeholder test as deterministic assertion",
  "instruction": "Return the exact final contents for tests/test_placeholder.py so the file is minimal and contains only a deterministic pytest test using assert 1 + 1 == 2.",
  "files": [
...
bash "${SCRIPT_DIR}/worker_codex_managed_single_file.sh" "${JOBS_DIR}/job-001.json" >/dev/null || true

STATUS_FILE="${RESULTS_DIR}/job-001.status.json"
TEST_FILE="${PROJECT_ROOT}/tests/test_placeholder.py"

echo "== Managed worker edit status =="
jq . "$STATUS_FILE"
echo ""
