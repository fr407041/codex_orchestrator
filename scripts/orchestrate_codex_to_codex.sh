...
You are the master planner for a low-context Codex-to-Codex orchestration system.

Goal:
Split the user task into at most ${MAX_JOBS} very small worker jobs for Codex CLI workers running on smaller open-source models.
...
- Optimize for avoiding token overflow.
...
for idx, job in enumerate(data.get("jobs", []), start=1):
    ...
    if not files:
        continue
    files = relativize_to_scope(files)
    payload = {
        "id": f"job-{idx:03d}",
...
if [[ "$EXECUTE_WORKERS" = "1" ]]; then
  for job_file in "$JOBS_DIR"/job-*.json; do
    ...
    worker_script="${SCRIPT_DIR}/worker_codex_cli.sh"
    if [[ "$WORKER_MODE" = "managed_single_file" ]]; then
      worker_script="${SCRIPT_DIR}/worker_codex_managed_single_file.sh"
...
PY
        for retry_job in "$JOBS_DIR"/"$(basename "${job_file%.json}")"-retry-*.json; do
          ...
          retry_worker_script="${SCRIPT_DIR}/worker_codex_cli.sh"
          if [[ "$WORKER_MODE" = "managed_single_file" ]]; then
            retry_worker_script="${SCRIPT_DIR}/worker_codex_managed_single_file.sh"
