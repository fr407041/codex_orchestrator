...
You are the worker Codex agent behind a master Codex orchestrator.
...
- If this task requires edits, use the Codex file editing capability or shell commands to perform the real change before answering.
...
STATUS_LINE="$(grep -Ei '^STATUS:\s*' "$RAW_FILE" | tail -n 1 || true)"
STATUS="SUCCESS"
if echo "$STATUS_LINE" | grep -Eiq '^STATUS:\s*OVERFLOW_DETECTED'; then
  STATUS="OVERFLOW_DETECTED"
elif echo "$STATUS_LINE" | grep -Eiq '^STATUS:\s*NEEDS_REPLAN'; then
  STATUS="NEEDS_REPLAN"
elif [[ $EXIT_CODE -ne 0 ]] || echo "$STATUS_LINE" | grep -Eiq '^STATUS:\s*FAILED'; then
  STATUS="FAILED"
elif echo "$STATUS_LINE" | grep -Eiq '^STATUS:\s*SUCCESS'; then
  STATUS="SUCCESS"
